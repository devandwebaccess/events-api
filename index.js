...
app.get("/scrape", async (req, res) => {
  try {
    ...
    $("li.sc-1nyzlro-1.jHDhvy").each((index, element) => {
      const date = $(element)
        .find("span.VisuallyHidden-sc-8buqks-0.lmhoCy span")
        .first()
        .text()
        .trim();
      const title = $(element)
        .find("span.sc-fyofxi-5.gJmuwa")
        .first()
        .text()
        .trim();
      const endDate = $(element)
        .find("span.sc-1idcr5x-0.bihyed span")
        .last()
        .text()
        .trim();
      const location = $(element)
        .find("span.sc-fyofxi-5.gJmuwa:nth-child(2)")
        .text()
        .trim();
      const state = $(element)
